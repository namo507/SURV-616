...
$X^{2}$ = (i = 1 to n) $\epsilon$  $($O_{i}$-$E_{i}$)^{2}$/$E_{i}$

where $O_{i}$ is the observed frequency (searches per week), and $E_{i}$ is the expected frequency under the null hypothesis.
...
```{r}
# Convert 'subscribed' to numeric if not already done
deposit_data$subscribed_num <- ifelse(deposit_data$subscribed == "yes", 1, 0)

# Fit a logistic regression model with 'campaign' as the only predictor
...
Ok so in practical terms, this suggests that increasing the number of campaign contacts is not effective in producing more subscriptions. In fact, it appears to have a diminishing return, where more contacts actually reduce the likelihood of a customer subscribing. This insight can help in optimizing marketing strategies by focusing on fewer, more impactful contacts rather than repeatedly contacting the same customers.

...
This is because cases (lung cancer patients) were deliberately oversampled by design. The sampling was based on disease status (outcome) rather than exposure status (smoking), which distorts the probability estimates. Therefore, we cannot estimate the true probability of lung cancer among smokers from these data.

In simpler terms: Since we artificially selected equal numbers of cases and controls rather than taking a random sample from the population, the calculated proportions don't reflect the true disease probabilities in the population.
...
Ok so first let me define the variables:
...
notbald_yes <- 548
notbald_no <- 7611
total <- bald_yes + bald_no + notbald_yes + notbald_no
```

1) First, calculating risks (incidence rates) for each group:

Risk for bald or Incidence in Bald (Exposed): 
$I_e = R_1 = \frac{n_{11}}{n_{11} + n_{12}} = \frac{127}{1,351} = 0.094$ or 9.4%

Risk for not bald or Incidence in Not Bald (Unexposed):
$I_u = R_0 = \frac{n_{21}}{n_{21} + n_{22}} = \frac{548}{8,159} = 0.067$ or 6.7%
```{r}
# Calculating risks
risk_bald <- bald_yes/(bald_yes + bald_no)
risk_notbald <- notbald_yes/(notbald_yes + notbald_no)
risk_bald
risk_notbald
```

2) Attributable Risk calculation (not considering the population for now):
$AR = I_e - I_u = R_1 - R_0$
$AR = 0.094 - 0.067 = 0.027$ or 2.7%

This means the excess risk of heart disease attributable to baldness is 2.7%.

3) Calculating Risk Ratio (RR):
$RR = \frac{R_1}{R_0} = \frac{0.094}{0.067} = 1.40$

4) Calculating proportion exposed (prevalence of baldness):
$p_e = \frac{n_{11} + n_{12}}{N} = \frac{1,351}{9,510} = 0.142$
```{r}
# Calculating RR and pe
rr <- risk_bald/risk_notbald
pe <- (bald_yes + bald_no)/total
rr
pe
```

5) Calculating Population Attributable Risk:
$PAR = \frac{p_e(RR-1)}{1 + p_e(RR-1)}$
$PAR = \frac{0.142(1.40-1)}{1 + 0.142(1.40-1)} = 0.054$ or 5.4%
```{r}
# Calculating PAR
...
6) For 95% CI calculation:

First, calculate Odds Ratio:
$OR = \frac{n_{11}n_{22}}{n_{12}n_{21}} = \frac{127 \times 7,611}{1,224 \times 548} = \frac{967,597}{670,752} = 1.442$

Taking natural logarithm:
$ln(OR) = ln(1.442) = 0.366$

Standard Error of log(OR):
$SE_{ln(OR)} = \sqrt{\frac{1}{n_{11}} + \frac{1}{n_{12}} + \frac{1}{n_{21}} + \frac{1}{n_{22}}}$

$SE_{ln(OR)} = \sqrt{\frac{1}{127} + \frac{1}{1,224} + \frac{1}{548} + \frac{1}{7,611}} = \sqrt{0.109} = 0.109$

95% CI for log(OR):
$ln(OR) \pm 1.96 \times SE_{ln(OR)}$
$0.366 \pm 1.96(0.109) = (0.152, 0.580)$

95% CI for OR:
$(e^{0.152}, e^{0.580}) = (1.164, 1.786)$

Now calculating incidence in unexposed (Iu):
$I_u = \frac{n_{21}}{n_{21} + n_{22}} = \frac{548}{548 + 7,611} = \frac{548}{8,159} = 0.067$

Calculating AR bounds:
$AR_{lower} = I_u(OR_{lower} - 1)$
$AR_{lower} = 0.067(1.164 - 1)$
$AR_{lower} = 0.067(0.164) = 0.012$ or 1.2%

$AR_{upper} = I_u(OR_{upper} - 1)$
$AR_{upper} = 0.067(1.786 - 1)$
$AR_{upper} = 0.067(0.786) = 0.051$ or 5.1%

Therefore, 95% CI for AR: (0.012, 0.051) or (1.2%, 5.1%)
...
# Calculating incidence in unexposed
Iu <- c/(c + d)

# Calculating AR bounds
AR_lower <- Iu * (ci_lower - 1)
AR_upper <- Iu * (ci_upper - 1)

cat("Odds Ratio:", round(OR, 3), "\n")
cat("95% CI for OR:", round(ci_lower, 3), "-", round(ci_upper, 3), "\n")
cat("95% CI for AR:", round(AR_lower, 3), "-", round(AR_upper, 3))
```

Based on my analysis of this prospective study data using R code, I found that baldness appears to have a moderate association with heart disease. The odds ratio of 1.441 suggests that individuals with baldness have about 44% higher odds of developing heart disease compared to those without baldness. I can be quite confident about this association because the 95% confidence interval (1.177, 1.764) doesn't include 1, indicating statistical significance.

Looking at the attributable risk confidence interval (0.012, 0.051), I can say that between 1.2% and 5.1% of heart disease cases in the population might be attributable to baldness. Interestingly, even at the lower bound of 1.2%, given how common heart disease is, this could represent a substantial number of cases that might be associated with baldness as a risk factor.